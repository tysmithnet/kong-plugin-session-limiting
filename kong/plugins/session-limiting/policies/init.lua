...
local policy_cluster = require "kong.plugins.session-limiting.policies.cluster"
local reports = require "kong.reports"


local ngx_log = ngx.log
local shm = ngx.shared.kong_session_limiting_counters
local pairs = pairs
...
local get_local_key = function(conf, identifier, period_date, name)
  ...
  if api_id == NULL_UUID then
    return fmt("sessionlimit:%s:%s:%s:%s:%s", route_id, service_id, identifier, period_date, name)
  end

  return fmt("sessionlimit:%s:%s:%s:%s", api_id, identifier, period_date, name)
end
...
return {
    ...
    increment = function(conf, limits, identifier, current_timestamp, value)
      ...
      for period, period_date in pairs(periods) do
        if limits[period] then
          ...
          if not newval then
            ngx_log(ngx.ERR, "[session-limiting] could not increment counter ",
  ...
  ["cluster"] = {
    increment = function(conf, limits, identifier, current_timestamp, value)
      ...
      if not ok then
        ngx_log(ngx.ERR, "[session-limiting] cluster policy: could not increment ",
                          db.name, " counter: ", err)
      end
  ...
  ["redis"] = {
    increment = function(conf, limits, identifier, current_timestamp, value)
      ...
      if times ~= 0 or conf.redis_database then
        ...
        -- plugin, and instances of the response-sessionlimiting plugin.
