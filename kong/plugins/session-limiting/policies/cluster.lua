  ...
  ["cassandra"] = {
    increment = function(db, limits, route_id, service_id, identifier, current_timestamp, value)
      ...
      for period, period_date in pairs(periods) do
        if limits[period] then
          ...
          if not res then
            log(ERR, "[session-limiting] cluster policy: could not increment ",
    ...
    find = function(db, route_id, service_id, identifier, current_timestamp, period)
      ...
      local rows, err = db:query([[
        SELECT * FROM sessionlimiting_metrics
        WHERE route_id = ?
          AND service_id = ?
          AND api_id = ?
          AND identifier = ?
  ...
  ["postgres"] = {
    increment = function(db, limits, route_id, service_id, identifier, current_timestamp, value)
      ...
      for period, period_date in pairs(periods) do
        if limits[period] then
          buf[#buf + 1] = fmt([[
            SELECT increment_session_limits('%s', '%s', '%s', '%s',
    ...
    increment_api = function(db, limits, api_id, identifier, current_timestamp, value)
      ...
      for period, period_date in pairs(periods) do
        if limits[period] then
          buf[#buf+1] = fmt([[
            SELECT increment_session_limits_api('%s', '%s', '%s', to_timestamp('%s')
            at time zone 'UTC', %d)
